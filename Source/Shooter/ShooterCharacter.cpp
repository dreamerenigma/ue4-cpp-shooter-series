...
AShooterCharacter::AShooterCharacter() :
	...
	// Camare interp location variables
...

{
	...
	CameraBoom->TargetArmLength = 180.f; // The camera follows at the this distance behind the character
	...
	// Don't rotate when the controller rotates. Let the controller only affect teh camera.
...
void AShooterCharacter::BeginPlay()
{
	...
	// Create FInterpLocation struct for aech interp location. Add to array
	...
	FHitResult& OutHitResult)
{
	...
	{
		// Out BeamLocation is the End location for the line trace
	}
...
{
	// Set cureent camera field of view
	if (bAiming)
	{
	...
	if (bShouldTraceForItems)
	{
		...
		if (ItemTraceResult.bBlockingHit)
		{
			...
			if (TraceHitWeapon)
			{
				...
				{
					// Not currently highlights a slots; highlight one
...
{
	// ceck to see if AmmoMAp contains Ammo's AmmoType
	if (AmmoMap.Find(Ammo->GetAmmoType()))
	{
		// Get amount of ammo in our AmmoMAp for Ammo's type
		int32 AmmoCount{ AmmoMap[Ammo->GetAmmoType()] };
		AmmoCount += Ammo->GetItemCount();
		// Set the amount in the MAp for this type
...
void AShooterCharacter::FinishReloading()
{
	...
	if (AmmoMap.Contains(AmmoType))
	{
		...
		{
			// Realoda the magazine with all the ammo we are carrying
...
void AShooterCharacter::FinishEquiping()
{
	if (CombatState == ECombatState::ECS_Stunned) return;
...
void AShooterCharacter::GetPickupItem(AItem* Item)
{
	...
	if (Weapon)
	{
		...
		else // Inventory is full! Swap with EquipWeapon
		{
			SwapWeapon(Weapon);
		}
	}
